...
### Per-session Docker sandboxes (`agent.sandbox.*`)
A session can run inside an isolated **Docker sandbox** instead of against the host workspace. The lifecycle is owned by `DockerSandboxManager` (integration) and exposed at `/api/sandboxes`:
- `POST /api/sandboxes` (optional `{ "image": "..." }`) starts a container, creates a per-session host directory under `$AGENT_HOME/sandboxes/<id>/workspace`, **bind-mounts it into the container at the same absolute path**, registers it with the `WorkspaceRegistry` under the new sandbox id, and returns a `SandboxInfo` (`id`, `containerName`, `image`, `status`, `workspace`). `GET /api/sandboxes`, `GET /api/sandboxes/{id}`, and `DELETE /api/sandboxes/{id}` (which `docker rm -f`s the container and removes its workspace) round out the CRUD. The container is kept alive with `tail -f /dev/null`.
- **Targeting a sandbox:** chat/stream requests carry the sandbox id on `AgentRequest.sandboxId`. `AgentService` resolves the workspace from `sandboxId` (falling back to `sessionId`) and threads it through `ToolContext.sandboxId()`. The *same* `/api/agent/chat` and `/api/agent/stream` endpoints are used — only the request gains the `sandboxId` field; when it is blank everything behaves exactly as before (default host workspace).
- **Where execution happens:** file tools (`read_file`/`write_file`/`edit_file`/`list_files`/`grep_search`/…) operate on the host-side bind-mount (so the `Workspace` path confinement still applies), while `run_bash` is routed **into the container** via `docker exec` so all shell execution is isolated. Because the mount is at the same absolute path on host and container, file edits and shell commands share one consistent workspace path.
- **The SPI boundary is preserved:** `agent-core` defines `SandboxRuntime` (`isActive` + `exec`) and `BashCommandTool` depends only on that interface (`Optional<SandboxRuntime>`); the Docker implementation lives in `integration`. When no runtime is present, or the request has no active sandbox, `run_bash` falls back to host execution. Keep Docker-specific code out of `agent-core`.
...
- `agent.sandbox.*` — per-session Docker sandboxes: `enabled` (default true, `SANDBOX_ENABLED`; master switch for `/api/sandboxes` and sandbox-routed `run_bash`), `image` (default container image, `SANDBOX_IMAGE`; must already be available to the Docker daemon — overridable per create request), `docker-bin` (`SANDBOX_DOCKER_BIN`), `exec-timeout`/`start-timeout`, `network` (`SANDBOX_NETWORK`; blank = default bridge, `none` to isolate), `container-prefix`, `max-sandboxes` (concurrency cap). Sandbox workspaces live under `$AGENT_HOME/sandboxes/<id>/`.
