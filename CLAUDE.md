...
- In-process tools the agent can call directly, with **no MCP-SDK dependency** so they live in `agent-core`: `read_file`, `write_file`, `edit_file`, `list_files`, `find_files`, `find_path`, `grep_search`, `run_bash`, `read_discord_channel`, `list_discord_commands`, and (when `memory.enabled`) `recall_memory`. Each implements `LocalTool` (discovered as Spring beans) and is collected by `LocalToolRegistry`. `recall_memory` is gated on `memory.enabled=true` (it depends on `MemoryService`) and lets the agent search long-term memory on demand; `read_discord_channel` surfaces up to the last 10 messages of the originating Discord channel from `ToolContext.channelMessages()` (the snapshot the bot sends on `AgentRequest.recentMessages`); `list_discord_commands` surfaces the apps (bots present in the guild) and the bot's own slash commands available in the originating channel from `ToolContext.channelApps()` (the snapshot the bot sends on `AgentRequest.channelApps`) — the bot gateway API only exposes Hugin's own application commands, so other apps' commands aren't listed. `find_path` is the forgiving "locate" tool — it finds files and directories by name or path fragment (case-insensitive, fuzzy) even when the exact path doesn't exist, so the agent can resolve approximate paths the user gives (e.g. "look for the folder /code/hugin/hugin"). `find_files` (glob) takes an optional `type` (`file`/`dir`/`any`) to match directories too.
...
- The tool's `execute(args, ctx)` receives a `ToolContext` carrying the request's `Workspace`, `sessionId` (the origin, e.g. `discord-channel-123`), **`channelMessages`** (the caller-supplied recent-message snapshot from `AgentRequest.recentMessages`, used by `read_discord_channel`), **and `channelApps`** (the caller-supplied apps/slash-commands snapshot from `AgentRequest.channelApps`, used by `list_discord_commands`). Tools that need to route something back to the caller use `ctx.sessionId()` as the reply-to target — see the scheduling tools below.
