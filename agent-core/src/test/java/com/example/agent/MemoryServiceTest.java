...
class MemoryServiceTest {
    ...
    void recallEmbedsQueryAndSearchesStore() {
        ...
        when(store.search(embedding, 3, 0.75)).thenReturn(List.of(hit));

        List<MemoryStore.ScoredMemory> result = memoryService.recall("what is the weather?");

        assertThat(result).containsExactly(hit);
        verify(store).search(embedding, 3, 0.75);
    }
    ...
    void recallSwallowsEmbeddingFailures() {
        ...
        assertThat(memoryService.recall("hello")).isEmpty();
    ...
    void rememberEmbedsExchangeAndStoresRecord() {
        ...
        memoryService.remember("hi", "hello there");
    ...
    void rememberSkipsBlankAnswers() {
        memoryService.remember("hi", "  ");
        memoryService.remember("hi", null);
        verifyNoInteractions(embeddingClient, store);
    }
    ...
    void rememberSwallowsStoreFailures() {
        ...
        org.mockito.Mockito.doThrow(new RuntimeException("redis down")).when(store).save(any());

        memoryService.remember("hi", "answer");
